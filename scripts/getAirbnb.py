...
def search(city, state, country, start, end, num_days):
...
def parseHtml(html):
    ...
    for i in range(1,len(dirty_prices), 2):
        prices.append((int)dirty_prices[i])

    for i in range(0, len(dirty_urls), 2):
        url_str= dirty_urls[i]
        temp = url_str.replace("data-url=", "")
        temp1 = temp.replace('"', '')
        urls.append(domain + temp1)

    #print(prices)
    #print(urls)
    #os.system("firefox " + urls[1])
    
    best_price = min(prices)
    index = prices.index(best_price)
    link = urls[index]
    total_cost = best_price * num_days
    
    data = {'airbnb_cost' : total_cost, 'url' = link}
...
def getAirbnbEntries(city, state, country, startdate, enddate):
    ...
    best_option = parseHtml(response_text)
